...
def test_region_links(admin_user_client, super_client, p1, p2, region_url):
    ...
    s1, stack1 = _create_env_and_svc(client1)
    s2, stack2 = _create_env_and_svc(client2)
...
def test_regions_setup(admin_user_client, super_client, p1, p2, p3,
                       region_url):
    cleanup_region_data(super_client, p1, p2)
    client1 = client_for_project(p1)
    assert len(client1.list_host()) > 0
    ...
    s1, stack1 = _create_env_and_svc(client1)
    s1 = client1.wait_success(s1)
    s1 = client1.wait_success(s1.activate())
    s2, stack2 = _create_env_and_svc(client2)
    s2 = client2.wait_success(s2)
    s2 = client2.wait_success(s2.activate())
    s3, stack3 = _create_env_and_svc(client3, labels={"foo": "bar"})
    s3 = client3.wait_success(s3)
    s3 = client3.wait_success(s3.activate())
    ...
    launch_config = {"imageUuid": get_haproxy_image()}
    stack = create_env(client1)
    port_rule = {"protocol": "tcp", "selector": "foo=bar",
                 "environment": p3.name,
                 "region": REGION_NAME,
    ...
    lb = client1.wait_success(lb)
    client1.wait_success(lb.activate())
...
def test_biderectional_links_setup(admin_user_client, super_client,
                                   p1, p2, region_url):
    cleanup_region_data(super_client, p1, p2)
    client1 = client_for_project(p1)
    assert len(client1.list_host()) > 0
    ...
    s1. \
        setservicelinks(serviceLinks=[link])
    link_svc = "%s/%s/%s/%s" % (REGION_NAME, p1.name, stack1.name, s1.name)
    link_alias = "mylink"
    link = {"service": link_svc, "name": link_alias}
    s2. \
        setservicelinks(serviceLinks=[link])
